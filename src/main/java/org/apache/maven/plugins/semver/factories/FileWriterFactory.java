...
public class FileWriterFactory
{
	 ...
	 */
	public static void createReleaseProperties(MavenProject project, Map<FINAL_VERSION, String> finalVersions)
	{
		String mavenProjectRelease =
				"project.rel." + project.getGroupId() + "\\\u003A" + project.getArtifactId() + "\u003D"
						+ finalVersions.get(FINAL_VERSION.RELEASE);
		String mavenProjectDevelopment =
				"project.dev." + project.getGroupId() + "\\\u003A" + project.getArtifactId() + "\u003D"
						+ finalVersions.get(FINAL_VERSION.DEVELOPMENT);
		String mavenProjectScm = "scm.tag=" + finalVersions.get(FINAL_VERSION.SCM);

		String releaseText = mavenProjectRelease + "\n" + mavenProjectDevelopment + "\n" + mavenProjectScm;
	...
	public static void backupSemverPom()
	{
		...
		try
		{
			...
			{
				logger.warn(format("Old pom.xml.semverBackup removed  : %s", pomXmlSemverBackup.getAbsolutePath()));
				delete(pomXmlSemverBackup.toPath());
			}
			copy(pomXmlOriginal.toPath(), pomXmlSemverBackup.toPath());

			logger.info(format("New pom.xml.semverBackup prepared  : %s", pomXmlSemverBackup.getAbsolutePath()));
	...
	public static void writeFileToDisk(String fileName, String fileContent)
	{
		if (fileName != null)
		{
			...
			try (FileWriter writer = new FileWriter(file); Writer output = new BufferedWriter(writer))
			{
				if (file.exists())
				{
					logger.info(format("Old file: [ %s ] removed      : %s", fileName, file.getAbsolutePath()));
					delete(file.toPath());
				}
