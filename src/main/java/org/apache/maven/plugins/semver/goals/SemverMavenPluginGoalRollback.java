...
public class SemverMavenPluginGoalRollback extends SemverMavenPlugin
{
	...
	@Inject
	public SemverMavenPluginGoalRollback(VersionProvider versionProvider, PomProvider pomProvider,
	...
	public void execute()
	{
		...
		if (getConfiguration().getRunMode() == NATIVE || getConfiguration().getRunMode() == NATIVE_BRANCH
		...
		{
			logger.error("");
			logger.error(format("Ÿou have configured a wrong RUN_MODE ( %s )", getConfiguration().getRunMode()));
