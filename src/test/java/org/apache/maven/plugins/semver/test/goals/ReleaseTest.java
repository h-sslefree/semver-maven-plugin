...
/**
 * Bij het runnen van de tests moet eerst het project met maven zijn gebuild LETOP de configuratie
 * van de plugin in test wordt niet geladen!!!!!
 *
 * @author sido
 */
public class ReleaseTest extends AbstractSemverMavenPluginTest {
  ...
  public void testSemverMavenPluginPatchRelease() throws Exception {
    SemverMavenPluginGoalPatch mojo =
        (SemverMavenPluginGoalPatch) mojoRule.lookupConfiguredMojo(loadPom("release/"), "patch");
  ...
  public void testSemverMavenPluginMinorRelease() throws Exception {
    SemverMavenPluginGoalMinor mojo =
        (SemverMavenPluginGoalMinor) mojoRule.lookupConfiguredMojo(loadPom("release/"), "minor");
    Assert.assertNotNull(mojo);
    mojo.execute();
  }
  ...
  public void testSemverMavenPluginMajorRelease() throws Exception {
    SemverMavenPluginGoalMajor mojo =
        (SemverMavenPluginGoalMajor) mojoRule.lookupConfiguredMojo(loadPom("release/"), "major");
    Assert.assertNotNull(mojo);
    mojo.execute();
  }
}
